...
- **Result**: replaced the "Planned: Redirect-Based Sign-In" section with the full synthesized design — concrete implementation code (with the bug fixed), the research findings with honest confidence levels, a staged verification protocol with explicit go/no-go gates, a rollout/migration strategy, UX copy for every step of the round trip, and a consolidated risk list. This was a planning-document-only change — no application code was touched.
- **Follow-up (same session)**: given the plan's own §3 flagged the Stage 0 device test as an almost-certain failure based on the weight of evidence, the user decided not to pursue the redirect approach at all rather than spend a device-testing cycle confirming what the research already pointed to. The entire "Planned: Redirect-Based Sign-In" section was removed from this README (it would otherwise describe a rewrite nobody intends to build); see Sessions 47-49 above for the full diagnosis and research if this is ever revisited. Effort instead redirected at improving the existing popup-based retry flow directly — see the next session entry.

#### Session 50: EXPERIMENT (v4.27) — "Next Natural Tap" Silent-Retry Escalation Instead of an Immediate Recovery Banner (Claude)
- **Request**: with the redirect rewrite ruled out (Session 49), reduce the existing popup-retry flow's visible-friction footprint without reintroducing an interactive account-picker prompt — the user explicitly rejected that path earlier in this same investigation as worse UX (fewer failed attempts, but it requires tapping Google's own visible picker UI, breaking the feeling of staying "within the app"), preferring today's two-taps-both-within-the-app model over a one-tap-but-leaves-the-app model.
- **Design** (discussed and approved before any code changed, per the user's explicit process): (1) the very first tap anywhere flips the calendar from its static skeleton to the real, properly-styled-but-data-less shell — pure "the app is responsive" feedback, independent of sign-in — and silently attempts sign-in with no toast and no banner; on an installed iOS PWA this attempt is expected to fail (the known popup-relay bug, unchanged from Sessions 35/47). (2) Rather than surfacing the recovery banner on that first, expected failure, the user's *next* ordinary tap — opening a calendar day, adding a transaction, anything, not a dedicated "Retry" tap — silently retries again, and only from this point does a "Connecting to Google…"/"Loading data…" toast appear. (3) After a small, capped number of these quiet retries (2) still fail, escalation falls back to the existing stuck-recovery banner, left **completely unchanged** (same dedicated "Retry sign-in" button, no tap-anywhere on the banner itself — locked in earlier this investigation and reaffirmed now).
- **Implementation**:
  - `auth.js`: added `Auth.silentRetry()` — mirrors `triggerFirstTapSync()`'s core `requestToken('')`/`_tokenReady` resolve-or-rearm logic, but has zero UI side effects (unlike `signIn()`, which unconditionally shows the full-screen "Reconnecting to Google" overlay — reusing it here would have visibly broken the "stays within the app" goal the whole redesign exists for). Also changed `triggerFirstTapSync()`'s failure branch to stop firing `notaTokenStuck`/`overlay.showSignIn()` immediately — that escalation decision now belongs to index.html's tap-counted logic instead.
  - `index.html`: rewrote the early first-tap-listener script (still its own `<script>` immediately after `auth.js`'s tag, for the same pre-`DOMContentLoaded` dead-tap-window reasons as Session 47) to add the skeleton→real-shell flag/event bridge (consumed by a new listener in the main `DOMContentLoaded` handler, since `initCalendar()`/`calInited` aren't defined yet at the point the early script runs), withhold the toast until the second attempt, and add the capped (`MAX_SILENT_RETRIES = 2`) re-arming retry listener that falls back to dispatching the existing `notaTokenStuck` event once exhausted.
  - Bumped `APP_VERSION` to `'v4.27'` specifically to mark this as the revert point, per the user's explicit request — if the real-device result is worse than the pre-experiment flow, revert `index.html`/`auth.js` to the commit just before this one.
- **Verification**: read the full pre-existing stuck-recovery-banner IIFE first to confirm this change doesn't touch its logic (`notaTokenStuck` listener, watchdog, button handler — all untouched). Sanity-checked via the local dev server preview (no OAuth possible in this sandboxed environment): page loads without script errors, `v4.27` shows in the footer, the calendar shell and stuck-recovery banner markup both render correctly. The actual popup-retry-timing behavior this experiment targets can only be verified on a real device — pending the user's follow-up screen recording.
