...
- **Follow-up**: the Session 48 toast was confirmed working in real-world testing shortly after (the "no toast in any frame" observation was deploy/cache lag, as suspected, not a code defect).

#### Session 49: Full Six-Dimension Analysis of the Redirect-Sign-In Plan — a Major Course Correction Before Any Implementation (Claude)
- **Request**: rather than start implementing the redirect-flow plan, the user asked for it to be thoroughly checked, its impact analyzed, and rewritten properly first — explicitly invoking multi-agent orchestration ("ultracode") for the scale of analysis warranted.
- **What was run**: a 13-agent Workflow — six parallel deep-dive analyses (state management/boot-sequence integration; security; iOS/WebKit platform research via live web search; service worker interaction + rollout strategy; verification protocol; UX during the round-trip), an adversarial critique pass over each of the six, then one synthesis pass reconciling contradictions between dimensions and merging everything into the complete section below.
- **The single most consequential finding**: live research into iOS/WebKit standalone-PWA behavior turned up strong, multi-year, cross-project community evidence (Apple Developer Forums, multiple unrelated open-source projects, 2018-2025) that a top-level navigation away from an installed Home Screen web app to a different origin **does not cleanly return to the standalone context** — it very likely kicks out to a normal Safari tab on both legs, and iOS's storage partitioning is separately reported to isolate `sessionStorage`/`localStorage`/cookies/the Service Worker registration between the standalone instance and that Safari-tab context. This directly threatens the entire premise of the redirect rewrite and the CSRF/PKCE `state` design that depends on `sessionStorage` surviving the round trip — not device-verified, but strong enough evidence that the plan now leads with a mandatory, cheap (~20-30 min, no OAuth client needed) go/no-go device test before any other implementation work, rather than treating platform verification as a nice-to-have footnote.
- **Other real corrections the synthesis made**: recommended Authorization Code + PKCE over implicit flow outright (Google has deprecated implicit, verified live against current docs, and GIS's own token client only supports the popup UX — implicit-via-redirect would require abandoning GIS entirely anyway, so it buys no simplicity); caught and fixed a logic bug in an early draft's proposed boot code (reusing `_deferredMode` as the redirect-detection signal would have made deferred mode permanently unreachable); retracted a proposed `localStorage` CSRF fallback after the platform research showed it doesn't actually solve the storage-isolation problem it was meant to solve; resolved two independently-proposed tap-gating designs into one concrete Tier-1/Tier-2 scheme (silent first-tap attempt stays exactly as it works today; only an explicit, dedicated tap can ever fire the actual cross-origin navigation, so an ordinary tap opening a calendar day can never accidentally redirect the user to Google).
- **Result**: replaced the entire "Planned: Redirect-Based Sign-In" section (below) with the full synthesized design — concrete implementation code (with the bug fixed), the research findings with honest confidence levels, a staged verification protocol with explicit go/no-go gates, a rollout/migration strategy, UX copy for every step of the round trip, and a consolidated risk list. This is a planning-document-only change — no application code was touched; the redirect flow itself remains unimplemented pending the Stage 0 device test.

## Planned: Redirect-Based Sign-In (Not Yet Implemented)

Scoped in Session 47, deliberately not started — this is a real architectural change, not a same-session fix, and this project's own recent history (Sessions 36–46) is a direct warning against attempting something this size without room to verify it properly. Session 49 did a full six-dimension analysis (state management, security, iOS/WebKit platform research, service worker/rollout, verification protocol, UX) plus an adversarial pass over each — this section is the synthesized result: a concrete design, not a set of open questions. Read it in full before touching code; several parts correct mistakes an earlier draft of this same plan made.

### 1. The idea, tightened

The confirmed root cause (Session 47, frame-by-frame screen recording) is that Google's popup-based sign-in relies on a `postMessage`/`window.opener` relay from the popup back to the parent page to deliver the token — and that relay is what's broken in WebKit's installed-standalone-PWA runtime, not the sign-in itself (the popup visibly reaches Google and starts loading a real response before dying, ~1.5–2s in). A full-page redirect flow doesn't use a popup or that relay at all: the whole tab navigates to Google and back, so there's no child window relay to break.

**What Session 49's research changed about this idea**: the original framing treated "does navigation away from an installed PWA stay inside the standalone window" as one open question to confirm before starting. It is not a minor open question — it is the single load-bearing assumption the entire rewrite depends on, and the evidence uncovered this session leans toward it being *false* on iOS specifically (see §3). This doesn't kill the idea, but it changes the shape of the plan: **Stage 0 below is now a hard go/no-go gate, run before any other line of this plan is implemented**, not a nice-to-have sanity check tucked into a "suggested approach."

### 2. Implementation design

#### 2a. Flow choice: Authorization Code + PKCE, not implicit — definitive, not a toss-up

Verified live against Google's current identity documentation this session:

- Google has **deprecated the implicit flow outright**. Its own docs (`developers.google.com/identity/protocols/oauth2/javascript-implicit-flow`) call it *"insecure for browser-based single-page applications (SPAs) due to the risk of token interception,"* say interacting with the endpoint directly is *"strongly discouraged,"* and point new work at Auth Code + PKCE instead.
- **The "implicit is simpler because there's no backend" argument doesn't hold for a redirect rewrite specifically.** GIS's `initTokenClient()`/`requestAccessToken()` — what `auth.js` uses today — **only supports the popup/dialog UX** (verified: *"Due to security concerns, only the dialog UX is supported"*). There is no redirect mode on the token client. Building implicit-via-redirect means hand-constructing the `accounts.google.com/o/oauth2/v2/auth?response_type=token&...` URL yourself, abandoning GIS for this step entirely. That is exactly the same amount of "leave the maintained library" work PKCE requires — implicit buys zero implementation simplicity here, only extra risk.
- **New attack surface unique to redirect, closed by PKCE and not by implicit**: today, nothing in `auth.js` trusts anything arriving via the address bar — the only token path is GIS's own callback, driven by this app's own `requestAccessToken()` call. A redirect flow, by construction, adds a boot path that trusts the URL. A crafted link mimicking Nota's own callback URL, if the app accepted any `access_token` it found in a fragment, would hand the *attacker's own* token into the victim's running app instance — a session-fixation attack where the victim's data starts flowing into a spreadsheet the attacker's account owns. PKCE closes this categorically: a bare `?code=...` handed to a victim is worthless without the matching `code_verifier`, which lives only in `sessionStorage` from a request the victim's own browser actually initiated.
- Threat-model check, taken seriously rather than waved at: the `drive.file` scope (`config.js:16`) means a stolen token can only touch files Nota itself created, and there's no backend/server session/cross-tenant blast radius. This lowers the *stakes* of either choice, but argues against over-engineering (don't stand up a real backend), not for picking the option Google itself calls insecure when the alternative costs the same amount of code.

**One test gates this before writing any boot-path code — not a hedge on the recommendation, a specific empirical unknown**: will `oauth2.googleapis.com/token` complete a PKCE exchange from a direct cross-origin browser `fetch()`, for a "Web application" GCP client type (the only type that accepts an arbitrary `https://` redirect URI — native client types are loopback/custom-scheme only) with no usable secret? Google's docs model this exchange as backend-only and never document CORS support for a pure client-side call. Build one throwaway HTML page, one `fetch()` call, before any `auth.js` work — see §5, Stage 1. If it's CORS-blocked, the fallback is **not** "fall back to implicit" (still deprecated, still carries the session-fixation exposure above) — it's either a minimal stateless serverless function whose only job is proxying that one POST (small, no user data, no state, arguably still compatible with the project's no-backend spirit), or staying on the current popup+recovery-banner investment.

This test is **secondary** to the iOS-navigation gate in §3/§5 — it answers "can PKCE work at all," not "does the redirect round trip survive on the one platform this whole rewrite targets." Run Stage 0 (§5) first.

#### 2b. State management — what changes, and where it lives across the reload

A full-page redirect means `auth.js`'s IIFE (`auth.js:39-348`) re-executes from scratch on the return leg — every `let`-bound closure variable (`accessToken`, `tokenExpiresAt`, `_deferredMode`, the `ready`/`_tokenReady` promises) resets. Nothing survives in memory; everything comes back from the URL or `sessionStorage`.

| State | Lifetime today | Redirect-flow source |
|---|---|---|
| `accessToken` / `tokenExpiresAt` | in-memory, set in `requestToken()`'s callback (`auth.js:174-175`) | result of the PKCE token-exchange POST on the return leg |
| CSRF `state` | doesn't exist today | `sessionStorage`, written before navigating, single-use (delete on first read regardless of outcome) |
| `code_verifier` | doesn't exist today | `sessionStorage`, same single-use treatment as `state` |
| `_deferredMode` | in-memory bool (`auth.js:70`), default `false` | still in-memory, but the redirect-return boot path is driven by a **separate**, purpose-built signal — see the bug fix below, not by overloading this flag |
| `spreadsheetId`/`opexGid`/`investGid` (`data-store.js:120-122`) | read once from `localStorage` at script load | **already reload-safe, no change needed** — the app already does full `location.reload()`s today (`signOut()` at `auth.js:344`; the 30-min inactivity timeout) and `data-store.js` survives both with zero special-casing |

**Corrected bug from the first draft of this plan**: an earlier version of this design reused `_deferredMode` itself as the "did the redirect just hand us a token" signal in the boot fork. That doesn't work — `_deferredMode` is `false` at the check point in *every* case (ordinary launch, denied/invalid redirect, and valid redirect all explicitly set or leave it `false` before the fork runs), which makes the "enter deferred mode" branch permanently dead code. Every returning user, redirect or not, would fall through to `requestToken('none')` — the exact call the code's own comment says is "expected to fail on iOS PWA" — landing on a full sign-in prompt on every single launch. **Use a distinct local flag** (`redirectTokenObtained`) instead, and never conflate it with `_deferredMode`.

**Capture-and-strip happens before `auth.js` ever runs, in the earliest possible inline script — not inside `start()`.** Why this placement, specifically: an authorization `code` arrives as a **query parameter** under PKCE (not a URL fragment, unlike the implicit flow this plan originally sketched) — and unlike a fragment, a query string genuinely is sent as part of the `Referer` header on any same-tick outbound subresource request (a font load, an analytics call, even the GIS script's own request) that fires before the string is stripped. That makes "first inline script in `<head>`, before the GIS `<script>` tag, before `config.js`, before `auth.js`, before anything else" a harder requirement under PKCE than it would have been under implicit's fragment-based token, not a softer one. Concretely, as the literal first script in `<head>`:

```html
<head>
<script>
(function() {
  // Must run before ANY other script — GIS's tag, config.js, auth.js, fonts,
  // everything — so a leftover ?code=...&state=... is captured and scrubbed
  // from the address bar before it can appear in a Referer header on any
  // other same-tick request. Bridges the captured values to auth.js via a
  // plain global, since this script runs long before auth.js's own IIFE does.
  var qs = new URLSearchParams(location.search);
  if (qs.has('code') || qs.has('error')) {
    window.__notaRedirectResult = {
      code: qs.get('code'),
      state: qs.get('state'),
      error: qs.get('error'),
    };
    var u = new URL(location.href);
    ['code', 'state', 'error', 'scope'].forEach(k => u.searchParams.delete(k));
    // replaceState, not location.hash='' / a second navigation — overwrites
    // the current history entry instead of leaving the code-bearing URL as a
    // separate, back-navigable entry.
    history.replaceState(null, '', u.pathname + u.search + u.hash);
  }
})();
</script>
<script src="config.js"></script>
<!-- GIS script tag, sheets-client.js, data-store.js, auth.js unchanged below -->
```

The strip is **unconditional** — it happens before any `state` validation, so even a forged, replayed, or malformed callback gets scrubbed from the URL just as eagerly as a valid one.

**The `start()` preamble** (replaces the placeholder "third boot path" framing from the first draft — this composes as a pre-fork state-seeding step ahead of the existing two-way fork, not a parallel branch, since modeling it as a genuine third branch would duplicate logic the fork already has):

```js
async function start() {
  const cachedSpreadsheetId = localStorage.getItem('notaPublic_spreadsheetId');
  const redirect = window.__notaRedirectResult; // set by the head-script above, else undefined
  let redirectTokenObtained = false;

  if (redirect) {
    const expectedState = sessionStorage.getItem(LS_OAUTH_STATE);
    const codeVerifier   = sessionStorage.getItem(LS_OAUTH_VERIFIER);
    sessionStorage.removeItem(LS_OAUTH_STATE);
    sessionStorage.removeItem(LS_OAUTH_VERIFIER); // single-use, regardless of outcome — this
                                                   // is what makes the check single-use, not
                                                   // just single-value; delete BEFORE assigning
                                                   // accessToken below, not after (a hot-reload
                                                   // or accidental double-parse must not be able
                                                   // to re-accept an already-consumed code)

    const valid = !redirect.error && expectedState && redirect.state === expectedState && codeVerifier;
    if (valid) {
      try {
        const tok = await exchangeCodeForToken(redirect.code, codeVerifier); // POST to oauth2.googleapis.com/token
        accessToken = tok.access_token;
        tokenExpiresAt = Date.now() + (Number(tok.expires_in || 3300) * 1000);
        redirectTokenObtained = true;
      } catch (err) {
        console.warn('[auth] redirect token exchange failed', err);
        // Fall through as an honest "sign-in didn't happen" — never fabricate success.
      }
    }
    // Denial, mismatched/missing state, or replay: fall through the same way, silently —
    // §6 defines the actual user-facing message for this case.
  }

  if (cachedSpreadsheetId) {
    if (redirectTokenObtained) {
      // Redirect already got us a token — skip deferred mode entirely.
      _deferredMode = false;
      readyResolve();
      tokenReadyResolve();
      return;
    }
    _deferredMode = true;               // unchanged returning-user path
    readyResolve();
  } else {
    overlay.showSplash();
    if (redirectTokenObtained) {
      // First-ever sign-in via redirect — run bootstrap() once, same as signIn() does today.
      try { await DataStore.bootstrap(); } catch (err) { console.error('[auth] bootstrap failed', err); }
      _deferredMode = false;
      readyResolve();
      tokenReadyResolve();
    } else {
      try {                              // unchanged new-user path
        await requestToken('none', 4000);
...
As a state diagram in prose: two independent booleans — "cached spreadsheet ID?" and "did this exact page load arrive with a valid token already in hand (`redirectTokenObtained`)?" — cross to produce four boot states, not a linear fourth path: (cached, no token) → today's unchanged deferred launch; (cached, token) → skip deferred mode, resolve both gates immediately; (no cache, no token) → today's unchanged splash/silent/button flow; (no cache, token) → run `bootstrap()` once, first-ever sign-in via redirect. **Whether a returning user's successful redirect return calls `DataStore.bootstrap()` matters and is easy to get backwards**: today `bootstrap()` runs only from `signIn()` (`auth.js:201`) and the new-user branch (`auth.js:315`), never on a returning user's silent token acquisition — despite `requireReady()`'s own comment (`data-store.js:370`) implying otherwise. Calling `bootstrap()` unconditionally on every redirect return would add real, unnecessary network calls (empty-cache check, migration-flag checks) to every reconnect, not just first-time sign-ins — the preamble above deliberately doesn't do that.

`requireReady()` (`data-store.js:369-372`) needs no changes — `await Auth.ready` plus the `spreadsheetId` check works identically regardless of how `ready` got resolved.

#### 2c. CSRF `state` and PKCE `code_verifier`

- `state`: `crypto.randomUUID()` — matches `data-store.js:16`'s existing convention, 122 bits of entropy.
- `code_verifier`: RFC 7636 wants 43–128 chars from `crypto.getRandomValues()`, base64url-encoded. A `crypto.randomUUID()` isn't directly reusable (wrong charset — strip the dashes, or use a dedicated one-line `getRandomValues()`-based generator).
- `code_challenge = base64url(SHA256(code_verifier))`, `code_challenge_method=S256` — always S256, never `plain`.
- **Compute the challenge eagerly, before the user taps, not inside the click handler.** `crypto.subtle.digest()` is async; a click handler that awaits it before calling `location.href = url` risks the click no longer being treated as a direct user gesture by the time the navigation fires (whether that risk is real specifically on iOS standalone WebKit is unverified — flag it, but don't build around an assumption either way). Sidestep the question entirely: generate `code_verifier`/`code_challenge` and write `state`+`code_verifier` to `sessionStorage` the moment the reconnect UI (overlay or recovery banner) is shown — *before* any tap happens — so the actual click handler that fires `location.href` is fully synchronous, referencing an already-computed URL.
- **Storage**: `sessionStorage`, written immediately before navigating, read + deleted (single-use) on return, deleted regardless of match/mismatch. **This is contingent on a question §3 has not resolved in the plan's favor — see the explicit caveat there before relying on this.**
- Never trust `code`/`access_token` from the URL without a matching, un-consumed `state` in storage. On mismatch or missing storage (e.g. the user reopened the callback URL from browser history rather than completing a fresh sign-in): identical treatment to a failed sign-in — the strip already happened unconditionally (§2b) — fall through to `overlay.showSignIn()`. Never silently proceed or auto-retry on mismatch.

#### 2d. Google Cloud Console configuration

- Add **"Authorized redirect URIs"** (today only "Authorized JavaScript origins" is configured, per the Setup section above) for **both** environments `config.js` already distinguishes — the exact production URL (protocol/host/path, no trailing-slash mismatch) and the localhost dev URL. The existing "Web application" client type is already correct — it's the only type that accepts an arbitrary `https://` redirect URI (native client types are loopback/custom-scheme only, per §2a).
- `access_type=online`, no `refresh_token` requested — keeps the storage model exactly as it is today: the token lives only in an in-memory `let accessToken`, re-derived per session, nothing new persisted to `localStorage`.
- `response_type=code`, `code_challenge_method=S256`, `scope=https://www.googleapis.com/auth/drive.file` (unchanged).
- No `client_secret` in the client-side code anywhere — this is a public client.

### 3. iOS/WebKit navigation research — confidence levels, stated honestly

**This is the single highest-leverage open question in the entire plan, and it is not yet resolved by anything short of a real-device test.** Multi-year community evidence points toward failure, but it is not device-verified, and treating it as settled — in either direction — would repeat exactly the mistake this project's own history warns against (Sessions 36-45: assuming WebKit behavior without a device test, more than once, and being wrong).

**What the research found, and how confident to be in each part:**

- **Top-level navigation out of an installed iOS standalone PWA very likely drops it into a normal Safari browsing context on both legs — departure and return.** iOS ties "standalone" rendering to the manifest's `scope`; navigating to a different origin (`accounts.google.com`) is documented and repeatedly, independently reported to exit standalone mode, and — critically — the *return* navigation lands in that Safari-tab context too, not back inside the original standalone window. This is corroborated across ~8 years of independent reports (2018 through 2023-2025) spanning several unrelated projects (next-auth, an Angular PWA seed, PocketBase, a Patreon integration) and Apple's own developer forums (threads 100407, 125109). **Confidence: strong, consistent community consensus — not an authoritative Apple/WebKit statement, and not device-verified in this session.** No confirmed prior art was found of anyone shipping a seamless version of exactly this; the most recent (2025) documented resolution people actually ship is "accept the Safari kick-out, have the user manually relaunch the Home Screen icon afterward, with the server storing the completed auth for pickup on the next standalone launch" — not the "token comes back in the URL fragment on next load, transparently" flow this plan originally assumed.
- **Storage isolation between the standalone instance and a same-origin Safari tab is well-documented and directly threatens the CSRF/PKCE design in §2c.** localStorage, sessionStorage, cookies, and even the Service Worker registration are reported **not shared** between the two contexts on iOS, despite being same-origin — only the Cache Storage API is confirmed shared. **This means the localStorage-based fallback an earlier draft of this plan proposed for the `state`/`code_verifier` problem does not actually solve it** — if the round trip crosses the standalone/Safari boundary, neither `sessionStorage` nor `localStorage` can be relied on to survive it. The only confirmed-shared channel (Cache Storage) is not a natural fit for a short-lived CSRF nonce and would need its own bespoke design if pursued. **Confidence: strong community consensus, multiply corroborated, not device-verified this session.**
- **A separate, thinner-sourced report**: a "PWA freezes on the home page after an OAuth redirect completes" failure mode on iOS 13+, five independent developer confirmations in one Apple Developer Forums thread (649699), 2020-2023, workaround only "background and re-foreground the app." **Confidence: plausible, worth watching for during the device test, not something to design around pre-emptively** — thinner evidence than the two findings above.
- **Apple's actual sanctioned mechanism for exactly this problem is `ASWebAuthenticationSession`** — a native-API-only, system-managed browser sheet that shares Safari's cookie jar and returns control via a completion handler, with no popup relay and no dependency on the standalone/Safari `scope` boundary at all. **Nota, as a plain Home Screen web app with no native wrapper, cannot call this API.** A thin native wrapper (e.g. Capacitor) would unlock it and plausibly resolve this class of problem outright — but that is a materially larger undertaking (App Store distribution, native build/review) than either the popup or redirect approach, is explicitly out of scope for this plan, and has no confirmed "someone did this for Google OAuth in a PWA" write-up to cite. Worth naming as the actual fallback option if Stage 0 below fails and popup-recovery investment is judged to have reached its ceiling.
- **No iOS 26 (current, 2026)-specific confirmation was found** — the underlying behavior has had at least one reported version-to-version wobble before (a partial, single-source-reported iOS 12.2 fix in 2019, followed by the same failure reported again in 13.5.1), so there is a real, if likely small, chance current-version behavior differs from the historical reports. This is exactly why Stage 0 (§5) is a required test, not a formality, and exactly why the correct framing here is "strong prior evidence of a probable failure, still requiring direct confirmation" — not a settled verdict in either direction.

**Bottom line for the implementer**: expect Stage 0 to fail, because that's what the weight of evidence points to — but run it, record the exact iOS version either way, and do not write a single line of the boot-path/CSRF code in §2 as production-committed until it passes. If it fails, the correct response is not "iterate on the redirect design" — it's to stop, per this project's own Session 46 lesson about not tuning around a broken premise, and either invest further in popup-failure recovery UX, or scope the native-wrapper path as a separate, larger initiative.

### 4. Service worker interaction

- **Fragments were never a service-worker concern under implicit; under PKCE, `code`/`state` arrive as query parameters, which the SW's `fetch` handler *does* see** — this is actually easier to defend at the SW layer than the fragment-based implicit design would have been, not harder.
- Traced against the live code: the redirect-return request is a normal top-level navigation for `index.html`, not in `BYPASS_HOSTS` (`sw.js:14-23`), not matching `data.json`/`cdnjs.cloudflare.com`, so it falls into `networkFirst(SHELL_CACHE, e.request)` (`sw.js:73`, defined at `sw.js:78-94`) exactly like any other load. When online — which the device necessarily just was, having reached Google — this fetches fresh and re-caches, so the return load isn't, by default, at special risk of a stale shell.
- **The real gap is `networkFirst`'s failure branch**: a network blip exactly as the tab returns (plausible on mobile, reconnecting after backgrounding for the Google trip) falls back to `caches.match(request)` — potentially a **pre-redirect-feature build with no idea how to parse a `code`/`state` return at all**, silently wasting the single-use code. Mitigation, made concrete by PKCE's query-string shape: special-case any request whose URL carries `code`/`state` params in the SW's `fetch` handler — on a network failure for that specific request shape, **never** fall back to `caches.match()`; return an explicit "you're offline, tap to retry" response instead. Silently serving a possibly-incompatible stale shell is worse than an honest failure for the one request where staleness is unusually costly.
- `BYPASS_HOSTS` **already includes `oauth2.googleapis.com`** (`sw.js:19`, alongside `accounts.google.com` at `sw.js:18`) — no service-worker change is needed to let the token-exchange POST through; this was sometimes described elsewhere as something to "add," it's already there.
- `SHELL_CACHE` version bump (`v4`→`v5`) is hygiene, not a mechanism — `networkFirst` refetches and overwrites the same cache entry on every successful load regardless of cache name; a bump only matters for `activate`'s stale-cache-name sweep and forcing a clean re-fetch across clients. Bump it as convention alongside this change.
- **Everything above assumes the redirect-return request lands in the same browsing context (and thus the same SW registration/cache partition) that served the departing page.** Per §3, that assumption is exactly the one under question — if a standalone→Safari-tab context split occurs, the return leg may hit a different SW registration entirely (or none yet), and this whole analysis doesn't necessarily transfer. **Extend Stage 0's device test (§5) to also check SW/Cache-Storage continuity across the round trip, not just `localStorage`/`sessionStorage`** — the same test, one more thing to observe.

### 5. Verification protocol — concrete, ordered steps

**Tooling limitation, stated up front**: none of this can be done with sandboxed browser automation — no `display-mode: standalone` emulation, no real interactive Google consent. Everything below requires a developer on a real iPhone.

**Prerequisites (blocking, before Stage 0):**
- Nothing — Stage 0 needs no OAuth client, no GCP config, no Nota changes. Deliberately built that way so the riskiest unknown gets tested cheapest and first.

**Stage 0 — bare navigation + storage/SW continuity smoke test (no Google, no PKCE, ~20-30 min).** This merges what were two near-duplicate test designs (the iOS-research dimension's Q5 test and this section's own original Stage 0) into one canonical script:
1. Install a simple static test page to the Home Screen; launch from the icon (not Safari). Confirm `navigator.standalone` and `matchMedia('(display-mode: standalone)').matches` are both `true`.
2. Add a debug button that sets `sessionStorage.setItem('t','1')`, `localStorage.setItem('t','1')`, and writes a marker into Cache Storage, then does `location.href = 'https://accounts.google.com/'` (a real different-origin page — no OAuth client needed yet, this is pure navigation/storage mechanics).
3. Observe: does Safari chrome (URL bar, tab UI) appear on arrival? That's the standalone-mode-lost signal.
4. Navigate back to the test page's own origin (a link, or `location.href`).
5. On return, check: is `navigator.standalone` still `true`? Is the UI still chrome-free? Are the `sessionStorage`/`localStorage`/Cache-Storage markers still readable? Is a Service Worker still registered and controlling the page?
6. **Repeat after a genuine cold launch** (force-quit the app, then reopen from the icon) — several reports specifically say the failure only reproduces after full termination/relaunch, not a warm session.
7. **Expect this to fail, per the weight of evidence in §3** — a failure confirms rather than surprises. If step 5 comes back fully clean on your specific device/iOS version, that is a genuine, notable exception to the current evidence; note the exact iOS version precisely, since it would be worth independently re-verifying rather than taken at face value.
8. **If this fails: stop.** Do not proceed to Stage 1. The whole redirect approach as designed is moot on this platform; escalate to the "what to do if Stage 0 fails" decision in §3/§7 rather than trying to route around it with a different storage mechanism.

**Stage 1 — PKCE CORS viability (only after Stage 0 passes, ~15 min).** One throwaway HTML page, one `fetch()` POST to `oauth2.googleapis.com/token` with a real `code`/`code_verifier` pair from a completed manual auth flow, for the actual "Web application" GCP client type this project uses. Confirms or refutes the open question in §2a before any `auth.js` work begins.

**Stage 2 — real OAuth round-trip, minimal wiring.** Point real navigation at the real Google endpoint with the real `client_id`/redirect URI from a debug build; complete a real sign-in; pair the screen recording with Safari Web Inspector over a Mac (Develop menu → device → the installed PWA's page is inspectable like any tab) for a console-log timeline, not video alone — Session 48's own unresolved video-only ambiguity (a shipped toast never visible in any frame, cause never pinned down) is the direct argument against relying on recordings alone.

**Stage 3 — full boot-path integration**, verified per the milestones and failure signatures below.

**Successful-run milestones** (no baseline exists yet for a redirect run — these establish one; pair every recording with console-log timestamps, e.g. `"redirect: navigating"`, `"redirect: return detected"`, `"redirect: strip complete"`, `"redirect: state valid"`, `"redirect: token exchange start/end"`, `"bootstrap start/end"`):
- M0 tap (on the explicit reconnect button — see §6) → M1 navigation away begins (expect sub-1-2s; flag any Safari-chrome flash even in an otherwise-successful run) → M2 Google's consent UI fully rendered (expect roughly comparable to the popup flow's own ~1.5-2s baseline) → M3 user completes consent (user-paced) → M4 redirect lands back with `code`/`state` actually present in `location.search`, **in the same standalone window** (log whether `navigator.standalone` is still `true` at this point — this is the single most important log line in the whole protocol) → M5a the query string is stripped in the first inline script, synchronously, unconditionally, regardless of validity (this must be near-instant and happen before M5b, not after) → M5b `state` is validated and the token-exchange POST completes → M6 `DataStore.bootstrap()` completes (first-sign-in case) or is correctly skipped (returning-user case), calendar shows real data → M7 total M0→M6, feeding the comparison in the baseline subsection below.

**Failure signatures:**
- **(a) Never comes back to standalone / kicked out to Safari** — `navigator.standalone` reads `false` on return, or Safari chrome persists. Disqualifying platform result per §3, not a code bug to fix.
- **(b) Comes back but wrong state** — token/code ignored, re-prompts sign-in, or `ready`/`_tokenReady` never resolve and the skeleton hangs. Boot-path integration bug (`Auth.start()`'s branching); diagnose via the log timeline.
- **(c) Comes back with a token but data doesn't load** — check the network tab (invisible on video alone) for CORS/token-exchange failures, a stale `spreadsheetId`, or a broken PKCE exchange.
- **(d) Works once, breaks on relaunch** — test ≥2 consecutive force-quit-and-relaunch cycles specifically; points at stale `sessionStorage` state, an incompletely stripped query string, or unchecked `tokenExpiresAt` on the second run.
- **(e) Forged/replayed/mismatched `state` or `code_verifier` is accepted rather than rejected.** Deliberately construct a callback URL with a missing or mismatched `state`, and separately a bare `code` with no matching `code_verifier` in storage, and confirm the app fails closed both times rather than accepting either — this is a security-critical case, not an edge case, and needs its own explicit pass, not just organic discovery.
- **(f) The user's original tap target is lost or hijacked.** With the Tier-1/Tier-2 design in §6, confirm an ordinary first tap (opening a calendar day, dismissing a toast) never itself triggers a cross-origin navigation, and that only an explicit tap on the reconnect button does.

**Regression matrix**: desktop browsers and Android (untouched, popup flow, must not regress) · iOS Safari plain tab (untouched or intentionally included, must not regress) · iOS installed standalone (the new path) — each crossed with brand-new user vs. returning-user (cached `spreadsheetId`), plus a sign-out→sign-in cycle to confirm `sessionStorage` state doesn't leak, plus a normal post-deploy launch confirming `sw.js`'s network-first `index.html` fetch still works and `BYPASS_HOSTS` already covers `oauth2.googleapis.com` (verified present, §4 — no addition needed there).

**Measured metrics, not just pass/fail** (both flagged as real, unaccounted-for costs by this session's analysis, and easy to miss if only timing is measured):
- **Redundant boot-render/fetch-cycle count.** Because a redirect return re-fires `DOMContentLoaded`, one user tap that triggers a redirect produces two full boot-render cycles and two `loadHistData()`/`fetchCurrentMonthFresh()` cycles where the popup flow does one (its own separate window never touches the main document's `DOMContentLoaded`). Count actual Sheets API calls per redirect sign-in and compare to one popup-flow sign-in — this is a real network/battery cost the plan doesn't currently optimize away, and should be an explicit, reported number, not just felt as "slightly slower."
- **A real "today" baseline for full tap→data-loaded, not just tap→banner.** The existing baseline measurements (kept below) only go as far as "popup closing → recovery banner appearing." The recovery banner's retry button calls `Auth.signIn()` unconditionally (`index.html:4013`), which reopens the *same* popup mechanism that's the entire premise of this investigation — so whether retry-in-place reliably reaches success at all, and in how long, is itself unmeasured. Establish this number during Stage 3 rather than assuming a "~35-65s" figure that no dimension actually measured.

**Decision rule** (mirrors Session 46's own revert criterion): keep the redirect flow only if failure signature (a) never occurs even once (disqualifying regardless of other numbers), first-attempt success rate is a clear, large improvement over today's near-zero first-attempt rate, median total tap→data-loaded time is no worse than today's real (now-measured, not assumed) retry-path total, and no (b)/(c)/(d)/(e)/(f) bugs remain after reasonable iteration — across **at least 5 real force-quit cold-launch recordings**, not 1 (the existing baseline data already shows one sub-step ranging 7s to 25-35s run-to-run; a single success proves nothing). Otherwise revert to the popup+recovery-banner approach and keep investing there. Log any visible Safari-chrome flash during an otherwise-successful run separately — even a technically-successful, standalone-preserving run can read to a user as "left the app," and that's worth weighing in the keep/revert call on its own.

### 6. Rollout and migration strategy

**Contingent on Stage 0 (§5) passing.** Everything below assumes the redirect round trip is confirmed, on a real device, to stay inside (or cleanly return to) the standalone context. If Stage 0 fails, this entire section is moot — don't build the flagging/deploy machinery for an approach that can't complete.

- **Gate on `Auth.isStandalone()`, applied uniformly to new and returning users — not on new-vs-returning status.** The popup-relay bug is rooted in the standalone runtime itself, not in which boot branch triggers the popup — `signIn()` (new-user overlay button, and the recovery banner's retry) and `triggerFirstTapSync()` (returning-user first tap) funnel through the identical `initTokenClient()`/`requestAccessToken()` machinery in the identical runtime. A brand-new user tapping "Sign in with Google" for the first time on an installed icon is exposed to the identical failure a returning user's first tap is. If anything, leaving new users on the broken popup path is worse: they have no cached, rendered spreadsheet to look at while recovering, unlike a returning user. Non-standalone users (desktop, Android, an un-installed browser tab) stay on the popup flow regardless of new/returning status — it already works there, and the redirect flow's extra surface area (full navigation away, state round-trip) is pure downside for a case that isn't broken.
- **`Auth.isStandalone()` needs to be promoted from a private helper into a real export.** It does not currently exist on `Auth` — the actual `isStandalone()` check (`navigator.standalone` / `display-mode: standalone`) lives as a private function inside index.html's stuck-recovery-banner IIFE (`index.html:3969-3972`), used only to pick the escape-hatch link's label. `auth.js`'s current exports are exactly `{ start, signIn, signOut, getAccessToken, triggerFirstTapSync, ready, markAppReady }` (`auth.js:347`) — no standalone detection. Move the check into `auth.js` and export it, since `auth.js` is what needs to decide the flow at `start()`/`signIn()`/`triggerFirstTapSync()` time.
- **Testing override: a localStorage-based debug flag, not a query param and not a random-percentage rollout.** A query-param override doesn't work for this specific case — an installed PWA launches from a fixed `start_url` via the Home Screen icon; there's no way to append `?authFlow=redirect` to that launch. A % rollout requires observing outcomes across an unknown bucket, but this app has no backend/telemetry — the entire evidence-gathering method that's actually worked on this bug class (Sessions 47-48) is one developer, one or two real devices, deliberately-triggered recordings, reproduced on demand. A random bucket works against that. Use a hidden debug affordance (e.g. a multi-tap on the existing Build label — `APP_VERSION`/`APP_BUILD` are already surfaced at `index.html:1401-1402`/`1491` for support diagnostics, a natural, already-half-built hook) that sets a `localStorage` override, checked at every call site alongside the code's own hardcoded `isStandalone()` default. Nothing is ever written to an ordinary user's `localStorage` automatically.
- **Safe deploy order, driven by two SW lifecycle facts**: `install` calls `skipWaiting()` and `activate` calls `clients.claim()` unconditionally already (no wait-for-old-clients phase), and `clients.claim()` only changes which SW handles a tab's *future* fetches — it doesn't reload JS already executing in an open tab. Since the redirect's outbound leg is a real navigation that destroys the page's JS context anyway, "old JS running against a new SW" isn't a hazard here. The real hazard: a user whose tab literally left for Google runs no SW-update check while away, so if a whole redirect-unaware→redirect-aware migration ships while they're mid-flight, they could return to code that never learned to look for `code`/`state` at all. Because that can't be fixed client-side, **ship the new `sw.js` + `index.html` (redirect-parsing code present but dormant behind the flag) as its own deploy first, let it sit for at least one full normal reopen cycle, and only flip the flag in a subsequent deploy.** That guarantees every user who can reach the redirect trigger is already running code that at least knows to look.
- **Mid-flight-deploy and rollback both degrade to today's status quo, not a new failure mode.** If a user returns to code new enough to parse but the `state` doesn't match (storage cleared, or the migration flip happened mid-flight), or to genuinely pre-feature code (the SW stale-shell case in §4) — both cases fall through to the existing, already-shipped deferred-mode + first-tap flow for a returning user. The token is simply wasted, not stranded. This is a strictly better position than Session 46's revert, which had to reconcile newly-introduced *runtime* logic (timeouts, `select_account`); here the only persisted artifacts of concern are a self-clearing `sessionStorage` entry and a stray, already-stripped URL — both inert.
- **Rollback**: flip the hardcoded standalone-branch default back to `popup`, redeploy via the existing GH Actions pipeline (single push to `main`, per this project's auto-commit-push convention). No GCP changes needed — leaving "Authorized redirect URIs" configured but unused is harmless. No per-user `localStorage` to clean up, since nothing is auto-persisted per user; only the developer's own manual debug-override key needs clearing, confined to 1-2 test devices.

### 7. UX copy and behavior for the redirect round trip

**The load-bearing fact that shapes most of this**: `index.html`'s cache-render path (`index.html:4075-4122`, `cachedHistLoaded && cachedCurMonth`) doesn't await `Auth.ready`, doesn't await a token, and reads straight from `localStorage` inside the `DOMContentLoaded` handler that already runs on *every* page load today. **A redirect round trip does not, by itself, turn a warm-cache boot into a cold-boot-looking experience** — the identical fast-render code runs again on return. What's genuinely new and costly is narrower: (1) the app UI visibly disappears during the trip in a way the popup never caused (the main document kept rendering the whole time, even while the popup was dying), and (2) mid-session token expiry (`getAccessToken()`'s silent-refresh-fails branch, `auth.js:285-293`) now discards in-progress UI state (a half-filled Add Transaction form, a scroll position) that a popup never touched.

**Deliberate-tap requirement — Tier 1 / Tier 2, resolving what two independent passes of this analysis converged on separately**: porting today's fire-from-any-first-click listener (`index.html:1390`, capture-phase, any click) unchanged would be actively dangerous under redirect — a tap meant to open a calendar day or dismiss a toast would yank the user out of the app to `accounts.google.com` before they did anything resembling "sign in." That's worse than today's silent-popup-attempt failure mode.
- **Tier 1 (any first tap, unchanged mechanism)**: keep the existing silent, non-navigating attempt exactly as it works today — it's cheap, doesn't open a lingering popup on failure, and is already known (per `auth.js`'s own comments) to fail fast on iOS PWA rather than hang. On success, proceed exactly as today. **On failure — the expected outcome for the standalone cohort this flow targets — do not auto-escalate into a cross-origin navigation from that same tap.**
- **Tier 2 (explicit reconnect tap only)**: show the existing overlay/recovery-banner button; only from that button's own click handler, synchronously (per §2c, using an already-precomputed PKCE challenge, no `await` ahead of the navigation call), fire the actual redirect. This is not a departure from the project's existing design principle — `signIn()`'s interactive request already only ever fires from `#notaSignInBtn`'s own click listener (`auth.js:112`), never automatically; the one real change is narrowing the first-tap-anywhere listener so it can never itself become the thing that triggers a cross-origin navigation. (A future enhancement — preserving and resuming whatever the user's original tap was trying to do across the round trip, rather than requiring a dedicated second tap — is worth naming as a nice-to-have, not required for a first working version.)

**Copy** (extends the existing voice — short, second person implied, tells the user what happens next):
- Just before redirecting: keep the button's existing synchronous `Connecting...` label change unchanged. Replace the overlay status line `'Opening Google sign-in…'` with: *"Taking you to Google to sign in — you'll be right back."*
- Returning-user reconnect entry point: extend the existing sentence (`auth.js:102-103`) rather than replace it: *"Reconnecting to Google — tap below to finish signing in. You'll leave Nota for a moment and come right back."*
- No new "welcome back" copy on a successful return — the existing toast chain (`Loading data…` → `✓ Updated`/`Synced ✓`) already covers it; inventing redirect-specific copy for a moment the user experiences as an ordinary relaunch would be noise.
- **User denies consent**: this is an expected, low-anxiety outcome, not an error — do **not** route it into the stuck-recovery banner (that copy, "Stuck? Retry," is written for "something hung" and would tell a user who correctly tapped Cancel that something is broken). Show instead: *"Sign-in was cancelled — tap below to try again. Nothing was shared with Nota."*
- **Generic error from Google** (misconfigured redirect URI, expired state, etc.): carry the existing pattern forward verbatim (`auth.js:209`): *"Sign-in failed: {message} — try again."*
- **Stale/interrupted redirect discovered on a later relaunch** (user closed the app during the Google step, never returned): don't invent a "something went wrong" banner. Write a small `notaPublic_pendingRedirect` timestamp to `localStorage` immediately before navigating away; clear it the moment a returning `code`/`state` is successfully consumed. If a fresh boot finds that flag set with no redirect result in the URL, clear it silently and fall through to the normal deferred-mode boot — no special messaging; from the user's point of view nothing broke, they just didn't finish last time.
- The existing stuck-recovery banner (watchdog + `notaTokenStuck`) stays the right mechanism for "reachable-but-broken" — no copy changes needed there.
- Land the redirect back on the **same** `index.html`, not a separate `callback.html` — a dedicated callback page needs its own duplicate boot logic (or a redirect-of-a-redirect), adding a second blank-flash hop for no benefit.
- **Mid-session-expiry resume context**: before navigating away from a mid-session reconnect (not the cold-launch or first-tap paths, which have nothing to restore), write a small `{ activeTab, formDraft }` object to `sessionStorage` keyed by the same `state` value, and restore it once the post-redirect boot completes. **This inherits the exact same unresolved dependency as the CSRF `state` check in §2c/§3 — if storage doesn't survive the round trip on the platform driving this whole rewrite, this feature silently never fires.** Build it, but don't treat "it fires reliably" as confirmed until Stage 0 passes; treat it as best-effort, not guaranteed.
- **Component the "blank flash" on return has two parts, and only one is verified**: the browser's own navigation-transition blank (its size on iOS standalone specifically is unverified — could be a plain reload-in-place, or could involve a visible Safari-chrome flash per §3), and Nota's own cache-render time (verified unchanged, fast, same code as today). Don't finalize marketing/reassurance copy about "you'll be right back" being seamless until Stage 0/2's actual recordings confirm which of those two is really happening.

### Risks

1. **iOS standalone navigation may not return to (or stay inside) the standalone context at all.** Strong, multi-year community consensus points toward failure; not device-verified. This is the top blocking risk and the reason Stage 0 (§5) exists as a hard gate before any other work. If it fails, the redirect approach as designed is not viable — the realistic options are further investment in popup-failure recovery, or a separate, much larger native-wrapper (Capacitor + `ASWebAuthenticationSession`) initiative.
2. **Storage-partition isolation between standalone and Safari-tab contexts undermines both the CSRF `state` and the PKCE `code_verifier` design.** localStorage is *not* a safe fallback for this (contrary to an earlier draft of this plan) — it's reported just as isolated as sessionStorage across this exact boundary; only Cache Storage is confirmed shared, and it isn't a natural fit for a short-lived nonce. This risk is subsumed by risk #1's Stage 0 test — a pass there effectively confirms this one too.
3. **PKCE token-exchange CORS viability from a pure static page is genuinely undocumented** for a "Web application" GCP client type with no secret. Blocking Stage 1 test (§5), sequenced after Stage 0, before any `auth.js` boot-path work.
4. **Double boot-render / double data-fetch cost per redirect sign-in** — one user tap produces two full `DOMContentLoaded` cycles and two `loadHistData()`/`fetchCurrentMonthFresh()` cycles, versus one for the popup flow (traced directly against the boot sequence, §2b/§5). Not a blocker, but a real, measurable network/battery cost that should be reported from Stage 3 testing, not silently accepted.
5. **A crafted link mimicking Nota's callback URL is new attack surface a popup flow never had** — mitigated categorically by PKCE's `code_verifier` requirement (§2a) plus mandatory `state` validation (§2c) that must gate every use of anything pulled from the URL, never optional or best-effort.
6. **`networkFirst`'s cache-fallback could serve a stale, redirect-unaware shell on a network blip during the return leg** — mitigated by the SW special-casing described in §4 (fail closed with an explicit retry message for any request carrying `code`/`state`, never silently fall back to `caches.match()` for that specific shape).
7. **Mid-session token-expiry resume-context (§6) may never fire reliably** on the exact platform it's meant to help, for the same reason as risk #2 — ship it, but as best-effort, and don't let its apparent completeness distract from the fact that its correctness is gated on the same unresolved question.
8. **Mid-flight deploy during a user's active redirect trip** — analyzed in §6 as bounded and self-recovering (degrades to today's already-shipped popup fallback, never a new stranded state), contingent on the same same-context assumption as risk #1/#2; not a standalone concern once Stage 0 passes.
9. **`Auth.isStandalone()` doesn't exist as an export yet** — a concrete prerequisite implementation task (promote out of the private IIFE at `index.html:3969-3972`), not a design risk, but easy to forget since the code currently only has the private version.

### Baseline measurements to compare against

(from real screen recordings, frame-extracted and read directly — see Sessions 47-48; not estimates), so a future implementation of the redirect flow has something concrete to beat:
...
**Parallel milestones for what a *successful* redirect run should look like** (from §5's protocol, to compare directly against the popup-flow numbers above once Stage 3 recordings exist):
- M0→M1 (tap to navigation beginning): expect sub-1-2s — no `waitForGis()` dependency once the URL is hand-built (§2a), no popup-open latency.
- M1→M2 (navigation to Google's consent UI rendered): expect roughly comparable to the popup flow's own ~1.5-2s, since it's the same underlying network round-trip to the same Google infrastructure.
- M4 (redirect lands back — the moment that determines pass/fail against risk #1): confirm `navigator.standalone` is still `true` at this instant, not just that `index.html` loaded.
- M5a (query-string strip): near-instant, synchronous, unconditional — should be effectively unmeasurable as a distinct step.
- M0→M6 (full tap-to-data-loaded): this is the number that must beat the *real*, now-actually-measured (not assumed) popup-flow tap-to-data-loaded baseline established during Stage 3, per §5's decision rule — not the older "tap→banner" partial baseline above, which was never a complete success measurement to begin with.
